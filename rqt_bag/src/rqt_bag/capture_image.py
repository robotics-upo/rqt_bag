...
from PyQt5.QtWidgets import QLabel, QRubberBand, QWidget, QMenu, \
    QMainWindow, QFileDialog, QLineEdit, QPushButton
    ...
    # OF TYPE RECT
        ...
        if len(self.sequence) > 0:
            ...
            for rectangle in self.sequence:
                self.currentQRubberBand = \
                    QRubberBand(QRubberBand.Rectangle, self)
        ...
        if eventQMouseEvent.buttons() == Qt.LeftButton:
            ...
            for rect in self.sequence:
                if rect.geometry().contains(eventQMouseEvent.pos()):
                    self.menu = MenuRectangle(self._timeline,
                            eventQMouseEvent, rect, self.imageLabel,
                            self.sequence)
                    found = True
            if not found:
...
class MenuGlobalImage(QMenu):
    ...
    # CREATION OF THE DROP-DOWN MENU WITH THE TAGS STORED IN THE TXT FILE
        ...
        with open('/home/roberott/Desktop/plugins/rqt_bag/src/rqt_bag/tags_file.txt'
                  , 'r') as f:
            line = f.readlines()
            for x in line:
        ...
        # TAG THE COMPLETE IMAGE
            ...
            if action.text() != 'Otra...' and action.text() \
                != 'Guardar' and action.text() != 'Limpiar todo':
        ...
        # TAG WITH A NEW TAG THE COMPLETE IMAGE
            ...
            if action.text() == 'Otra...':
                ...
                self.window.show()
                self.exec_()
            if action.text() == 'Limpiar todo' and len(self.sequence) \
                > 0:
                for rect in self.sequence:
            ...
            if action.text() == 'Guardar':
                if CaptureImage.bag == None:
                    ...
                    if filename[0] != '':
                        ...
                        record_filename = \
                            time.strftime('%Y-%m-%d-%H-%M-%S.bag',
                                time.localtime(time.time()))
                    if prefix.endswith('.bag'):
                        prefix = prefix[:-len('.bag')]
                    if prefix:
                        record_filename = '%s_%s' % (prefix,
                                record_filename)

                    bag = rosbag.Bag(record_filename, 'w')
                    CaptureImage.bag = bag
                    bag.write(self.topic + '/label', self.imageLabel,
                              self.stamp)
                    bag.close()
                    txt_file = open('/home/roberott/txt_file', 'w')
                    txt_file.write('imageTopic: '
                                   + self.imageLabel.imageTopic + '\n')
                    for rect in self.imageLabel.rect:
                        txt_file.write(' X: ' + str(rect.topX) + ' Y: '
                                + str(rect.topY) + ' Width: '
                                + str(rect.width) + ' Height: '
                                + str(rect.height) + ' Label: '
                                + str(rect.label) + '''

''')
                    txt_file.write(' Global: '
                                   + str(self.imageLabel.globalImageLabel))
        ...
        # ELIMINO LOS RECTANGULOS DIBUJADOS
            ...
            if action.text() == 'Limpiar':
                ...
                for rectangle in self.imageLabel.rect:
                    if rectangle.topX == rect.x() and rectangle.topY \
                        == rect.y():
...
class App(QMainWindow):
    ...
    def on_click(self):
        ...
        tags_file = \
            open('/home/roberott/Desktop/plugins/rqt_bag/src/rqt_bag/tags_file.txt'
                 , 'a')
